...
class ProductList extends React.Component {
  ...
  addItemAsGuest(productId){
    axios.post("/api/orders/0/lineItems", { productId })
      .then(res => res.data)
      .then(item => {
        this.setState({
          guestcart: this.state.guestcart.concat(item)
        })
      })
  }
  ...
  componentDidMount(){
    axios.get("/api/orders/0/lineItems")
      .then(res => res.data)
      .then(cart => {
        this.setState({ guestcart: cart })
      })
  }

  handleAdd(userId, productId){
    if(!userId){
      dispatch(this.addItemAsGuest(productId));
      // save this to the store
    }
    dispatch(this.props.addItem(userId, productId))
  }
                ...
                <div className="mb-3 col-sm-4" key={product.id}>
                  <div className="card">
                    <div className="card-body">
                      ...
                      <Link className="btn m-2 btn-success float-left" to={`/orders/${user.id}/lineItems`}
                        onClick={() => this.handleAdd(user.id, product.id)}>Add to Cart</Link>
                    </div>
                  </div>
                </div>
              )
...
const mapStateToProps = ({ products, user, cart, guestcart }) => {
  console.log('cart ', cart)
  return {
    products,
    user,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleAdd: (userId, productId) => {
      console.log("productId", productId, "guestcart:", guestcart)
      if(!userId){
        dispatch(this.state.addItemAsGuest(productId))
      }
      dispatch(addItem(userId, productId))
